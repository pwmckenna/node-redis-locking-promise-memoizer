...
var getRandomKey = function () {
    var defer = q.defer();
    crypto.randomBytes(48, defer.makeNodeResolver());
    return defer.promise.then(function (buf) {
...
module.exports = function () {
    ...
    return function memoize (fn, ttl) {
        ...
        var getRandomKeyRequest = getRandomKey();
        return function () {
            var context = this;
            var args = Array.prototype.slice.call(arguments);
            var argumentsKey = hash(args.map(JSON.stringify).join(","));

            return q.resolve().thenResolve(getRandomKeyRequest).then(function (functionKey) {
